...
export function DocumentosSection({ token, expedienteId, tareaId, vencimientoId, movimientoId, onCreated }: Props) {
  ...
  useEffect(() => {
    api.get<Documento[]>("/documentos", token, queryParam)
      .then(setDocs)
      .catch(() => {})
      .finally(() => setLoading(false));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  if (loading) return (
    <div className="space-y-3 animate-pulse">{[...Array(2)].map((_, i) => <div key={i} className="h-12 bg-ink-50 rounded-xl" />)}</div>
  );
    ...
    <div className="space-y-4">
      ...
      {docs.length > 0 && (
        <div className="flex items-center justify-between">
          ...
          {pdfCount >= 2 && expedienteId && (
            <span className="text-xs text-ink-400">{pdfCount} PDFs</span>
          )}
        </div>
      )}

      {docs.length === 0 ? (
        <div className="text-center py-6"><p className="text-sm text-ink-400">Sin documentos adjuntos</p></div>
      ) : (
          ...
          {docs.map((doc, index) => (
            <div key={doc.id} className="flex items-center gap-2 px-3 py-3 hover:bg-ink-50/50 transition-colors group">
              <div className="flex flex-col gap-0.5 flex-shrink-0">
                <button onClick={() => moveDoc(index, "up")} disabled={index === 0} className="w-6 h-6 flex items-center justify-center rounded text-ink-300 hover:text-ink-700 hover:bg-ink-100 disabled:opacity-20 transition" title="Mover arriba">
                ...
                <button onClick={() => moveDoc(index, "down")} disabled={index === docs.length - 1} className="w-6 h-6 flex items-center justify-center rounded text-ink-300 hover:text-ink-700 hover:bg-ink-100 disabled:opacity-20 transition" title="Mover abajo">
              ...
              <div className="flex-1 min-w-0">
                {editingLabelId === doc.id ? (
                  ...
                  <div className="cursor-pointer group/label" onClick={() => { setEditingLabelId(doc.id); setLabelDraft(doc.label ?? ""); }} title="Click para editar etiqueta">
