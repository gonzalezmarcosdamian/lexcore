...
export function MovimientoDetailSheet({ movimientoId, token, onClose, onDeleted, onUpdated }: Props) {
  ...
  useEffect(() => {
    Promise.all([
      api.get<Movimiento>(`/movimientos/${movimientoId}`, token),
      api.get<Nota[]>(`/movimientos/${movimientoId}/notas`, token),
    ]).then(([m, ns]) => {
      setMov(m);
      setNotas(ns);
      if (m.expediente_id) {
  ...
  const handleToggleEstado = async () => {
    ...
    const updated = await api.patch<Movimiento>(`/movimientos/${movimientoId}`, { estado: nuevoEstado }, token);
    setMov(updated);
    onUpdated(updated);
  };
  ...
  const handleAddNota = async () => {
    ...
    setSavingNota(true);
    try {
      const n = await api.post<Nota>(`/movimientos/${movimientoId}/notas`, { texto: notaTexto.trim() }, token);
  ...
  const handleDeleteNota = async (notaId: string) => {
    ...
    try {
      await api.delete(`/movimientos/${movimientoId}/notas/${notaId}`, token);
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await api.delete(`/movimientos/${movimientoId}`, token);
      onDeleted(movimientoId);
      onClose();
    } finally { setDeleting(false); }
  };

  const vencida = mov ? mov.fecha < todayAR() && mov.estado !== "cumplido" : false;
  const urgente = mov ? (() => { const diff = new Date(mov.fecha).getTime() - Date.now(); return diff >= 0 && diff < 48 * 3600 * 1000; })() && mov.estado !== "cumplido" : false;
  const headerCls = mov?.estado === "cumplido" ? "bg-green-50/40 border-green-100" : vencida ? "bg-red-50/40 border-red-200" : urgente ? "bg-amber-50/40 border-amber-200" : "bg-white border-ink-100";
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 bg-white rounded-t-2xl shadow-2xl max-h-[92vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 px-4 pb-8 space-y-4">
          {loading ? (
            ...
            <p className="text-sm text-red-500 py-4">No se pudo cargar el movimiento.</p>
          ) : (
            <>
              {editing ? (
                  ...
                  <div className="px-4 pt-4 pb-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                          <button
                            ...
                            className={`inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg border transition active:scale-95 ${
                              mov.estado === "cumplido" ? "bg-green-600 text-white border-green-600" : "bg-white text-ink-600 border-ink-300"
                            }`}
                          >
                            {mov.estado === "cumplido"
                        ...
                        <h2 className={`text-base font-bold leading-snug ${mov.estado === "cumplido" ? "line-through text-ink-400" : "text-ink-900"}`}>
                          {mov.titulo}
                        </h2>
              ...
              <div className="bg-white rounded-2xl border border-ink-100 p-4">
                ...
                <DocumentosSection movimientoId={movimientoId} token={token} />
              </div>
...
export const VencimientoDetailSheet = MovimientoDetailSheet;
