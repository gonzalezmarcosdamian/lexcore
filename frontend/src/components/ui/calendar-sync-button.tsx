...
export function CalendarSyncButton({ variant = "compact" }: Props) {
  ...
  // Solo mostramos si es usuario Google (ya tiene token de Calendar)
  if (!isGoogleUser || !token) return null;
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="w-8 h-8 rounded-xl bg-blue-50 flex items-center justify-center flex-shrink-0">
            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
              <path d="M19.5 3h-2.25V1.5h-1.5V3h-7.5V1.5h-1.5V3H4.5A1.5 1.5 0 003 4.5v15A1.5 1.5 0 004.5 21h15a1.5 1.5 0 001.5-1.5v-15A1.5 1.5 0 0019.5 3zm0 16.5h-15V9h15v10.5zM4.5 7.5V4.5h2.25V6h1.5V4.5h7.5V6h1.5V4.5H19.5V7.5h-15z" className="fill-blue-500"/>
            </svg>
          </div>
          <div>
            ...
            {result ? (
              ...
              <p className="text-xs text-red-500">{error.includes("calendar_id") || error.includes("configurado") ? (
                <Link href="/perfil" className="underline">Configurá tu calendario en Perfil</Link>
              ) : error}</p>
            ) : (
  ...
  // compact — solo ícono + texto corto, para headers
  return (
    <button
      onClick={handleSync}
      disabled={syncing}
