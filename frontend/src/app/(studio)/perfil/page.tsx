...
function PerfilPageInner() {
  ...
  const handleCheckout = async (plan: string) => {
    ...
    try {
      ...
      // Error técnico de MP — loguear pero no mostrar al usuario (WhatsApp ya está en los botones)
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4 pb-20 lg:pb-8">
      ...
      >
        ...
        {subsLoading ? (
          ...
          <div className="space-y-5">
            ...
            {subsMsg && (
              <div className={`px-4 py-3 rounded-xl border text-sm flex items-start justify-between gap-3 ${subsMsg.type === "ok" ? "bg-green-50 border-green-100 text-green-700" : "bg-red-50 border-red-100 text-red-700"}`}>
                <span>{subsMsg.text}</span>
                {subsMsg.type === "err" && (
                  <a
                    href="https://wa.me/5492920445362?text=Hola%2C%20quiero%20suscribirme%20a%20Luthor"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-shrink-0 inline-flex items-center gap-1.5 bg-green-500 hover:bg-green-600 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition"
                  >
                  ...
                  {planes.map((plan) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isCurrent ? (
                          <div className="w-full text-sm font-semibold py-2 rounded-xl bg-ink-100 text-ink-400 text-center cursor-default">
                            Plan actual
                          </div>
                        ) : (
                          <a
                            href={`https://wa.me/5492920445362?text=Hola%2C%20quiero%20suscribirme%20al%20plan%20${encodeURIComponent(plan.label)}%20de%20Luthor`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="w-full flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white text-sm font-semibold py-2 rounded-xl transition"
                          >
      ...
      >
        ...
        {isCalendarConnected ? (
          /* ── Estado: token disponible → mostrar calendarios y sync ── */
          <div className="space-y-4">
            <div className="flex items-center gap-2 text-sm text-green-700 font-medium">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
              </svg>
              {isGoogleUser ? "Conectado via tu cuenta de Google" : "Google Calendar conectado"}
            </div>
            ...
            {loadingCalendars ? (
              ...
              <div className="space-y-2">
                <label className={labelCls}>¿A qué calendario querés sincronizar?</label>
              ...
              <p className="text-xs text-ink-400">No se pudieron cargar tus calendarios. Intentá desconectar y volver a conectar.</p>
            )}

            {/* Botón sync — solo si hay calendario elegido */}
            {profile?.google_calendar_id && (
              <div className="bg-ink-50 rounded-xl p-4 space-y-2">
                <p className="text-xs text-ink-500">Sincronizá todos tus vencimientos pendientes como eventos en Google Calendar.</p>
                <button
                  onClick={handleSyncCalendar}
                  disabled={syncing}
                  className="flex items-center gap-2 bg-brand-600 hover:bg-brand-700 text-white rounded-xl px-4 py-2 text-sm font-semibold transition shadow-sm disabled:opacity-50"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                  </svg>
                  {syncing ? "Sincronizando…" : "Sincronizar vencimientos ahora"}
                </button>
              </div>
            )}
            ...
            <button
              ...
              className="text-xs text-ink-400 hover:text-red-500 font-medium transition disabled:opacity-50"
            >
