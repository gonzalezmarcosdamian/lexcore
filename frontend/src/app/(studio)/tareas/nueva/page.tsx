...
const TIPOS: { value: TareaTipo; label: string; icon: string }[] = [
  ...
  { value: "extrajudicial",  label: "Extrajudicial",  icon: "🤝" },
  { value: "administrativa", label: "Administrativa", icon: "📋" },
  { value: "operativa",      label: "Operativa",      icon: "⚙️" },
];

const inputClass = "w-full bg-white border border-ink-200 rounded-xl px-4 py-2.5 text-sm text-ink-900 placeholder-ink-400 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition";
const labelClass = "block text-sm font-medium text-ink-700 mb-1.5";

function NuevaTareaInner() {
  ...
  return (
    <div className="max-w-2xl mx-auto py-6 px-4 space-y-6 pb-28">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-ink-900">Nueva tarea</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-ink-100 shadow-sm p-6 space-y-5">
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button
            type="submit"
            disabled={saving}
            className="flex-1 text-sm font-semibold bg-blue-600 hover:bg-blue-700 text-white px-4 py-3 rounded-xl transition shadow-sm disabled:opacity-50"
          >
