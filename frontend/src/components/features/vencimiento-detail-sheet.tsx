...
export function VencimientoDetailSheet({ vencimientoId, token, onClose, onDeleted, onUpdated }: Props) {
  ...
  const [editForm, setEditForm] = useState({ descripcion: "", fecha: "", hora: "", tipo: "vencimiento" });
  const [savingEdit, setSavingEdit] = useState(false);
  const textareaRef = useRef<HTMLTextAreaElement>(null);

  useEffect(() => {
    Promise.all([
      api.get<Vencimiento>(`/vencimientos/${vencimientoId}`, token),
      api.get<Nota[]>(`/vencimientos/${vencimientoId}/notas`, token),
    ]).then(([v, ns]) => {
      setVenc(v);
      setNotas(ns);
      if (v.expediente_id) {
  ...
  const handleToggleCumplido = async () => {
    if (!venc) return;
    const updated = await api.patch<Vencimiento>(`/vencimientos/${vencimientoId}`, { cumplido: !venc.cumplido }, token);
    setVenc(updated);
    onUpdated(updated);
  };
  ...
  const handleSaveEdit = async () => {
    ...
    setSavingEdit(true);
    try {
      const updated = await api.patch<Vencimiento>(`/vencimientos/${vencimientoId}`, {
  ...
  const handleAddNota = async () => {
    ...
    setSavingNota(true);
    try {
      const n = await api.post<Nota>(`/vencimientos/${vencimientoId}/notas`, { texto: notaTexto.trim() }, token);
  ...
  const handleDeleteNota = async (notaId: string) => {
    ...
    try {
      await api.delete(`/vencimientos/${vencimientoId}/notas/${notaId}`, token);
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await api.delete(`/vencimientos/${vencimientoId}`, token);
      onDeleted(vencimientoId);
      onClose();
    } finally { setDeleting(false); }
  };

  const vencida = venc ? venc.fecha < todayAR() && !venc.cumplido : false;
  const urgente = venc ? (() => { const diff = new Date(venc.fecha).getTime() - Date.now(); return diff >= 0 && diff < 48 * 3600 * 1000; })() && !venc.cumplido : false;

  const headerCls = venc?.cumplido ? "bg-green-50/40 border-green-100" : vencida ? "bg-red-50/40 border-red-200" : urgente ? "bg-amber-50/40 border-amber-200" : "bg-white border-ink-100";
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 bg-white rounded-t-2xl shadow-2xl max-h-[92vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 px-4 pb-8 space-y-4">
          {loading ? (
            ...
            <p className="text-sm text-red-500 py-4">No se pudo cargar el vencimiento.</p>
          ) : (
            <>
              {/* Header card */}
              <div className={`rounded-2xl border overflow-hidden ${headerCls}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        <button
                          ...
                          className={`text-xs font-semibold px-2.5 py-1 rounded-full transition active:scale-95 ${venc.cumplido ? "bg-green-100 text-green-700" : "bg-purple-100 text-purple-700"}`}
                        >
                      ...
                      <h2 className={`text-base font-bold leading-snug ${venc.cumplido ? "line-through text-ink-400" : "text-ink-900"}`}>
                        {venc.descripcion}
                      </h2>
                    </div>
                ...
                <div className="px-4 pb-3 divide-y divide-ink-50 border-t border-ink-50">
                  ...
                  {exp && (
                    <div className="flex justify-between items-start gap-3 py-2.5">
                      ...
                      <Link href={`/expedientes/${exp.id}`} onClick={onClose} className="text-sm font-medium text-brand-600 text-right truncate max-w-[200px]">
              ...
              <div className="bg-white rounded-2xl border border-ink-100 p-4">
                ...
                <DocumentosSection vencimientoId={vencimientoId} token={token} />
              </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/40">
          <div className="bg-white rounded-t-2xl shadow-xl w-full max-h-[92vh] overflow-y-auto p-5 space-y-4">
            ...
            <div className="flex gap-3 pt-1">
              ...
              <button onClick={handleSaveEdit} disabled={savingEdit || !editForm.descripcion.trim() || !editForm.fecha} className="flex-1 bg-brand-600 text-white rounded-xl py-2.5 text-sm font-semibold disabled:opacity-50">{savingEdit ? "Guardando…" : "Guardar"}</button>
            </div>
          </div>
        </div>
      )}
